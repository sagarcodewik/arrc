...
export default function MarketsPage() {
  ...
  /** 🔥 LIVE UPDATED DATA */
  const assets = useLivePrices(data);
  ...
  return (
    <div className="min-h-screen bg-[#0A1220] text-white px-6 py-8 space-y-8">
      ...
      <div className="bg-[#101A2B] p-6 rounded-xl">
        ...
        {assets.map((item) => (
          <div key={item._id} className="flex justify-between bg-[#0A1220] p-4 rounded-lg mb-3">
            <div>
            ...
            <div className="text-right">
              <div className="font-semibold">${item.currentValue.toFixed(2)}</div>
              <div className={`text-sm ${item.profitLoss >= 0 ? "text-green-400" : "text-red-400"}`}>
                {item.profitLoss >= 0 ? "+" : "-"}${Math.abs(item.profitLoss).toFixed(2)}
              </div>
            </div>
          </div>
        ))}
