      ...
      <AnimatePresence>
        ...
        {open && (
          <>
            ...
            />
            <motion.aside
              className="fixed left-0 top-0 z-50 h-full w-72 bg-white shadow-xl ring-1 ring-black/5 lg:hidden"
              initial={{ x: "-100%" }}
              animate={{ x: 0 }}
              exit={{ x: "-100%" }}
...
function SidebarContent({ onClose }: { onClose?: () => void }) {
  ...
  const baseItemClasses =
    "flex w-full items-center gap-2 rounded-md px-3 py-2 text-sm font-medium transition-all duration-200 hover:bg-slate-100";
    ...
    <div className="flex h-full flex-col bg-blue-glass">
        ...
        <div className="relative group">
          <div className="relative h-[6.21rem] w-[6.21rem] rounded-full p-2 border-2 border-cyan-200/80 group-hover:border-cyan-100 overflow-hidden shadow-inner bg-black transition-all duration-300">
            ...
            <img
              src="/images/logo.png"
              alt="Logo Images"
              className="relative z-10 h-full w-full rounded-full object-cover transition-transform duration-300 group-hover:scale-110"
              style={{
      ...
      <div className="flex-1 overflow-y-auto p-4 text-gray-700 space-y-4">
        <div className="flex w-full flex-col">
          <div className="mb-3 flex h-6 items-center rounded-md px-2 text-xs font-semibold text-slate-600">
            Main Navigation
          </div>
      ...
      <ul className="mt-auto border-t border-gray-100 p-4 space-y-1">
        ...
        <li>
          <button
            type="button"
            onClick={handleLout}
            className="group bg-white flex items-center gap-2 rounded-xl p-2 text-sm font-medium transition-all duration-200 hover:bg-slate-100 w-full"
          >
